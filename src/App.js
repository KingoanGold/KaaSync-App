...
const GAMES_DATA = {
  ...
  secretChallenges: ["Porte des sous-vêtements sexy dehors sans me le dire.", "Envoie-moi un message explicite quand je ne m'y attends pas.", "Ce soir, prends totalement les commandes au lit.", "Glisse un mot coquin dans ma poche aujourd'hui."]
};
...
const TIPS_DATA = [
  ...
  { id: 't2', title: "La Musique idéale pour le lit", cat: "Sensorielles", icon: <Music/>, time: "4 min", content: "La musique peut transformer une expérience banale en un moment magique..." },
  { id: 't3', title: "Réussir les positions debout", cat: "Pratique", icon: <Wind/>, time: "3 min", content: "Les positions debout ou acrobatiques nécessitent quelques précautions..." },
  { id: 't4', title: "L'art délicat de l'Aftercare", cat: "Émotionnel", icon: <Heart/>, time: "3 min", content: "L'aftercare (les soins post-coïtaux) est crucial, surtout après un rapport intense..." },
  ...
  { id: 't6', title: "Les zones érogènes méconnues", cat: "Sensorielles", icon: <Sparkles/>, time: "5 min", content: "Ne foncez pas directement vers les zones génitales ! Prenez le temps d'explorer..." },
  { id: 't7', title: "Introduire des jouets dans le couple", cat: "Pratique", icon: <Zap/>, time: "4 min", content: "Un sex-toy ne remplace pas un partenaire, c'est un outil pour explorer..." },
  { id: 't8', title: "L'art du Teasing (Faire monter le désir)", cat: "Préliminaires", icon: <Timer/>, time: "3 min", content: "Le sexe commence bien avant d'être dans la chambre. L'anticipation est clé..." },
  ...
  { id: 't10', title: "Initiation au Bondage Léger", cat: "Découverte", icon: <Lock/>, time: "4 min", content: "Attacher son partenaire (ou l'être) crée un abandon total très excitant..." }
];

const POSITIONS_DATA = [
  { n: "Le Missionnaire (L'indémodable)", c: "Face à face", d: 1, s: 1, desc: "Le partenaire A s'allonge sur le dos, les jambes légèrement écartées. Le partenaire B se place au-dessus en appui sur les mains ou les avant-bras.", v: "Variante : Le partenaire A referme complètement ses jambes autour de celles du partenaire B." },
  { n: "Le Missionnaire surélevé", c: "Face à face", d: 2, s: 2, desc: "Position classique du missionnaire, mais les jambes du partenaire allongé reposent sur les épaules de celui qui est au-dessus.", v: "Variante : Glissez un gros coussin sous les fesses du partenaire allongé." },
  ...
  { n: "L'Andromaque", c: "Au-dessus", d: 2, s: 3, desc: "Le partenaire A est allongé sur le dos. Le partenaire B s'assoit à califourchon face à lui.", v: "Variante : Le partenaire du dessus peut se pencher en avant et s'appuyer sur le torse." },
  { n: "La Cuillère", c: "De côté", d: 1, s: 2, desc: "Les deux partenaires sont allongés sur le flanc, emboîtés l'un dans l'autre (en cuillère).", v: "Variante : Le partenaire arrière glisse un bras sous la nuque du receveur." },
  { n: "Le Poteau", c: "Debout & Acrobatique", d: 4, s: 4, desc: "Le receveur se tient debout, le dos fermement plaqué contre un mur. Le partenaire actif se tient debout face à lui.", v: "Variante : Le receveur lève une jambe et l'enroule autour de la hanche." },
  ...
  { n: "Le 69 Classique", c: "Oral & Préliminaires", d: 2, s: 5, desc: "Les partenaires sont allongés tête-bêche, l'un sur le dos, l'autre par-dessus.", v: "Variante : Synchronisez vos mouvements de langue." },
  { n: "Le Missionnaire Jambes au ciel", c: "Angles & Tweaks", d: 2, s: 4, desc: "Le receveur garde les jambes parfaitement droites et tendues vers le plafond, posées contre le torse de l'actif.", v: "Variante : L'actif pousse doucement les chevilles vers sa tête pour cambrer le dos." },
  { n: "Le Papillon de Nuit", c: "Sensorielles", d: 1, s: 5, desc: "Plongez la chambre dans le noir le plus total. Bandez les yeux de l'un des partenaires.", v: "Variante : Murmurez des instructions érotiques." }
];

const FULL_CATALOG = POSITIONS_DATA.map((p, i) => ({ id: `p${i}`, name: p.n, cat: p.c, diff: p.d, spice: p.s, desc: p.desc, v: p.v }));
...
export default function App() {
  ...
  const [showIdeaModal, setShowIdeaModal] = useState(false); // NOUVEAU BOITE À IDÉES
  const [showPartnerProfile, setShowPartnerProfile] = useState(false); // NOUVEAU PROFIL DUO
  const [adminPopup, setAdminPopup] = useState(null); // NOUVEAU POPUP ADMIN
  ...
  const [ideaText, setIdeaText] = useState(''); // NOUVEAU BOITE À IDÉES
  ...
  const lastCommandRef = useRef(Date.now()); // NOUVEAU ADMIN
    ...
    if (typeof window !== 'undefined' && 'Notification' in window && Notification.permission === 'granted') {
      try {
        ...
        if (navigator.serviceWorker && navigator.serviceWorker.ready) {
          navigator.serviceWorker.ready.then(reg => reg.showNotification(title, { body }).catch(e=>e)).catch(e=>e);
        }
  ...
  // --- ÉCOUTE DES DONNÉES UTILISATEUR ET DU PARTENAIRE ---
  useEffect(() => {
    if (!user) return;
    ...
    const unsubUser = onSnapshot(userRef, (snap) => {
      if (!snap.exists()) {
        const initial = { 
          ...
          lastIntimacy: 0,
          pingToPartner: 0,
          lastActive: Date.now() // NOUVEAU ADMIN
        };
        ...
        updateDoc(userRef, { lastActive: Date.now() }).catch(e=>e); // Met à jour le statut en ligne
        ...
        if (data.partnerUid) {
            ...
            if (pSnap.exists()) {
              ...
              newlyLiked.forEach(likedId => {
                if (myLikesRef.current.includes(likedId)) {
                    ...
                    fireSystemNotification("Match Parfait ! 🔥", `${pData.pseudo || 'Votre partenaire'} a aimé la même position que vous !`);
                }
              });
          ...
          const qChat = query(collection(db, 'artifacts', appId, 'chats', chatId, 'messages'), orderBy('createdAt', 'desc'), limit(1));
  ...
  // --- NOUVEAU : ÉCOUTE DES COMMANDES ADMIN ---
  useEffect(() => {
    if (!user) return;
    const qCmd = query(collection(db, 'artifacts', appId, 'admin', 'commands'), orderBy('timestamp', 'desc'), limit(1));
    const unsubCmd = onSnapshot(qCmd, (snap) => {
      if (!snap.empty) {
        const cmd = snap.docs[0].data();
        if (cmd.timestamp > lastCommandRef.current) {
          lastCommandRef.current = cmd.timestamp;
          
          if (!cmd.targetUid || cmd.targetUid === user.uid || cmd.targetUid === 'ALL') {
            if (cmd.type === 'POPUP_MESSAGE') {
              setAdminPopup(cmd.text);
            } else if (cmd.type === 'FORCE_NAV') {
              setActiveTab(cmd.tab);
              setActiveGame(null);
            } else if (cmd.type === 'FORCE_EVENT') {
              setActiveTab('jeux');
              setActiveGame(cmd.gameType);
              triggerGameResult(cmd.gameType);
  ...
  const positionDuJour = useMemo(() => FULL_CATALOG[Math.floor((new Date() - new Date(new Date().getFullYear(), 0, 0)) / 1000 / 60 / 60 / 24) % FULL_CATALOG.length], []);
  ...
  const getDaysSinceIntimacy = () => {
    const maxDate = Math.max(userData?.lastIntimacy || 0, partnerData?.lastIntimacy || 0);
  ...
  const handleGoogleLogin = async () => {
    try { await signInWithPopup(auth, new GoogleAuthProvider()); } catch (error) { notify("Erreur lors de la connexion", "❌"); }
  };

  const requestNotificationPermission = async () => {
    if (!('Notification' in window)) return notify("Votre navigateur ne supporte pas les notifications.", "❌");
    try {
      const permission = await Notification.requestPermission();
      if (permission === 'granted') { setNotificationsEnabled(true); notify("Notifications activées avec succès !", "🔔"); }
    } catch (e) { notify("Erreur d'activation.", "❌"); }
  };
  ...
  const handleLike = async (id) => {
    ...
    await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid), { likes: isLiked ? userData.likes.filter(l => l !== id) : arrayUnion(id) });
    if (!isLiked && partnerData?.likes?.includes(id)) {
  ...
  const handleSavePosition = async () => {
    ...
    setIsCreating(false); setEditPosId(null); setNewPos({ name: '', cat: 'Face à face', newCat: '', desc: '', v: '', diff: 3, spice: 3, shared: true });
  ...
  const handleLinkPartner = async () => {
    ...
    } catch (error) { notify("Une erreur est survenue.", "❌"); }
  };

  const handleUnlinkPartner = async () => {
    if (!user) return;
    if (window.confirm("⚠️ Attention : Voulez-vous vraiment vous séparer de ce partenaire ?")) {
  ...
  const handleSubmitIdea = async () => {
    ...
    try {
      ...
      notify("Idée envoyée avec succès 💡", "✅");
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(query(collection(db, 'artifacts', appId, 'chats', chatId, 'messages'), orderBy('createdAt', 'asc'), limit(50)), (snap) => {
  ...
  const handleSendMessage = async (e) => {
    ...
    setNewMessage(''); 
    await addDoc(collection(db, 'artifacts', appId, 'chats', [user.uid, userData.partnerUid].sort().join('_'), 'messages'), { text: msgText, uid: user.uid, createdAt: Date.now() });
  };
  ...
  if (loading) return <div className="fixed inset-0 bg-slate-950 flex items-center justify-center text-rose-500"><Flame className="animate-pulse" size={48} /></div>;
  ...
  // --- NOUVEAU : ECRAN DE VERROUILLAGE ADMIN ---
  if (userData?.lockUntil > Date.now()) {
    return (
      <div className="fixed inset-0 bg-slate-950 flex flex-col items-center justify-center p-6 text-center z-[9999]">
        <Lock size={80} className="text-rose-500 mb-8" />
        <h1 className="text-3xl font-black text-white mb-4">Accès Restreint</h1>
        <p className="text-slate-400 max-w-sm">{userData.lockReason || "L'application a été temporairement verrouillée par l'administrateur."}</p>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 text-slate-100 flex flex-col font-sans overflow-hidden" style={{ WebkitTapHighlightColor: 'transparent' }}>
      ...
      <header className="px-6 border-b border-white/5 flex items-center justify-between bg-slate-950/80 backdrop-blur-xl z-50 shrink-0" style={{ paddingTop: 'max(env(safe-area-inset-top), 1.25rem)', paddingBottom: '1.25rem' }}>
        <div className="flex items-center gap-2 text-rose-500 font-black text-2xl tracking-tighter"><Flame fill="currentColor" size={28} className="drop-shadow-[0_0_15px_rgba(244,63,94,0.5)]" /> KAMA<span className="text-white">SYNC</span></div>
      ...
      <div className="fixed top-28 left-1/2 -translate-x-1/2 z-[100] flex flex-col gap-2 pointer-events-none items-center w-full px-4">
        {notifications.map(n => <div key={n.id} className="bg-slate-800/90 backdrop-blur-md text-white px-6 py-3 rounded-2xl text-xs font-bold shadow-2xl flex items-center gap-3 border border-white/10"><span className="text-lg">{n.icon}</span> {n.msg}</div>)}
      </div>
          ...
          <div className="animate-in fade-in duration-500">
            ...
            <div className="px-6 mb-8 space-y-4">
              <div className="relative group">
                ...
                <input type="text" placeholder="Rechercher une position..." className="w-full bg-slate-900 border border-slate-800 rounded-3xl py-4 pl-14 pr-4 outline-none focus:border-rose-500 text-base" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </div>
              <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  ...
                  {(searchQuery || filterSpice > 0 || filterPhysique > 0 || filterCat !== 'Toutes' || sortBy !== 'az') && <button onClick={resetFilters} className="text-[10px] font-black text-rose-500 uppercase">Réinitialiser</button>}
                </div>
                <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
                  <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-slate-900 border border-slate-800 rounded-xl px-4 py-2 text-[10px] font-bold text-emerald-400 min-w-[120px] text-base"><option value="az">Trier : A-Z</option><option value="spice">Trier : Plus épicé</option><option value="diff">Trier : Moins physique</option></select>
                  <select value={filterCat} onChange={(e) => setFilterCat(e.target.value)} className="bg-slate-900 border border-slate-800 rounded-xl px-4 py-2 text-[10px] font-bold text-slate-300 min-w-[120px] text-base"><option value="Toutes">Catégories</option>{displayCategories.map(c => <option key={c.id} value={c.id}>{c.id}</option>)}</select>
                </div>
              </div>
            </div>
                      ...
                      return (
                        <div key={pos.id} onClick={() => setSelectedPosition(pos)} className={`relative snap-center shrink-0 w-48 bg-gradient-to-br ${category.color} border ${isMatch ? 'border-amber-500/50' : 'border-white/5'} rounded-[2rem] p-5 cursor-pointer hover:scale-[1.02] transition-all`}>
                          ...
                          <div className="flex justify-between items-start mb-4">
                             <div className="flex flex-col gap-1"><div className="flex gap-0.5 mt-1">{[...Array(5)].map((_, i) => <div key={i} className={`w-1 h-1.5 rounded-full ${i < pos.spice ? 'bg-rose-500' : 'bg-white/10'}`}></div>)}</div></div>
        ...
        {activeTab === 'jeux' && !activeGame && (
          <div className="animate-in fade-in duration-500 p-6 mt-4">
             ...
             <div className="grid grid-cols-1 gap-4">
               {[{id: 'truthOrDare', icon: <Zap size={18} className="text-rose-500"/>, t: "Action ou Vérité", d: "Confessions intimes et défis charnels."},
                 {id: 'loveDice', icon: <Dices size={18} className="text-amber-500"/>, t: "Dés de l'Amour", d: "Laissez le hasard dicter vos caresses."},
                 {id: 'scenario', icon: <Shuffle size={18} className="text-purple-500"/>, t: "Scénario Aléatoire", d: "Lieu + Rôle + Twist inattendu."},
                 {id: 'roulette', icon: <Timer size={18} className="text-pink-500"/>, t: "Roulette Préliminaires", d: "Tâches sensorielles pour faire monter le désir."},
                 {id: 'secretChallenge', icon: <Gift size={18} className="text-emerald-500"/>, t: "Défi Secret (24h)", d: "Un défi à accomplir en cachette."}].map(g => (
                 <button key={g.id} onClick={() => setActiveGame(g.id)} className="bg-slate-900 border border-slate-800 rounded-3xl p-5 text-left flex items-center justify-between group hover:bg-slate-800 transition">
                   <div><h3 className="font-bold text-white flex items-center gap-2 mb-1">{g.icon} {g.t}</h3><p className="text-xs text-slate-400">{g.d}</p></div>
            ...
            <div className="flex-1 overflow-y-auto p-6 pb-32 flex flex-col items-center justify-start pt-8 text-center custom-scroll">
              {activeGame === 'truthOrDare' && (
                <div className="w-full max-w-md">
                  ...
                  {gameResult ? <div className="bg-rose-900/20 border border-rose-500/30 p-8 rounded-[2rem] animate-in zoom-in duration-300 mb-8"><h3 className="text-xl font-bold text-white leading-relaxed">{gameResult}</h3></div> : <p className="text-slate-400 mb-8">Osez révéler vos secrets ou passez à l'action.</p>}
              ...
              {activeGame === 'loveDice' && (
                <div className="w-full max-w-md">
                  ...
                  {gameResult ? <div className="bg-amber-900/20 border border-amber-500/30 p-8 rounded-[2rem] animate-in zoom-in duration-300 mb-8"><h3 className="text-xl font-bold text-white leading-relaxed whitespace-pre-line">{gameResult}</h3></div> : <p className="text-slate-400 mb-8">Laissez les dés choisir votre prochaine étape.</p>}
              ...
              {activeGame === 'scenario' && (
                <div className="w-full max-w-md">
                  ...
                  {gameResult ? <div className="bg-purple-900/20 border border-purple-500/30 p-8 rounded-[2rem] animate-in zoom-in duration-300 mb-8"><h3 className="text-lg font-bold text-white leading-relaxed whitespace-pre-line text-left">{gameResult}</h3></div> : <p className="text-slate-400 mb-8">Prêts à jouer un rôle ce soir ?</p>}
              ...
              {activeGame === 'roulette' && (
                <div className="w-full max-w-md">
                  ...
                  {gameResult ? <div className="bg-pink-900/20 border border-pink-500/30 p-8 rounded-[2rem] animate-in zoom-in duration-300 mb-8"><h3 className="text-xl font-bold text-white leading-relaxed">{gameResult}</h3></div> : <p className="text-slate-400 mb-8">Faites monter la température lentement.</p>}
              ...
              {activeGame === 'secretChallenge' && (
                <div className="w-full max-w-md">
                  ...
                  {gameResult ? <div className="bg-emerald-900/20 border border-emerald-500/30 p-8 rounded-[2rem] animate-in zoom-in duration-300 mb-8"><h3 className="text-lg font-bold text-emerald-400 mb-4 uppercase text-[10px] tracking-widest">À réaliser d'ici demain</h3><p className="text-xl font-bold text-white leading-relaxed">{gameResult}</p></div> : <p className="text-slate-400 mb-8">Tirez un défi personnel à accomplir en cachette de votre partenaire.</p>}
            ...
            <div className="grid grid-cols-1 gap-4 mt-8">
              {TIPS_DATA.map(tip => (
                <div key={tip.id} onClick={() => setSelectedTip(tip)} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex items-start gap-4 cursor-pointer hover:bg-slate-800/50">
                  ...
                  <div><h3 className="font-bold text-white text-base mb-1">{tip.title}</h3><p className="text-slate-400 text-xs line-clamp-2">{tip.content}</p></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="animate-in fade-in duration-500 p-6 mt-4">
             ...
             {!userData?.partnerUid ? (
               ...
               <div className="space-y-6">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                   <div onClick={() => setIsChatOpen(true)} className="bg-slate-900 border border-slate-800 p-4 rounded-[2rem] flex items-center gap-3 cursor-pointer hover:bg-slate-800 transition-all shadow-lg">
                     <div className="bg-rose-500/20 p-3 rounded-full text-rose-500 shrink-0"><MessageSquare size={20} /></div>
                     <div className="flex-1"><h3 className="font-bold text-white text-xs">Chat Secret</h3></div>
                   </div>
                   
                   {/* NOUVEAU : BOUTON PROFIL DUO */}
                   <div onClick={() => setShowPartnerProfile(true)} className="bg-slate-900 border border-slate-800 p-4 rounded-[2rem] flex items-center gap-3 cursor-pointer hover:bg-slate-800 transition-all shadow-lg">
                     <div className="bg-indigo-500/20 p-3 rounded-full text-indigo-500 shrink-0"><User size={20} /></div>
                     <div className="flex-1"><h3 className="font-bold text-white text-xs">Son Profil</h3></div>
                   </div>
                 </div>
                 ...
                 <div className="bg-slate-900 border border-slate-800 rounded-[2rem] p-6">
                    ...
                    <div className="flex justify-between items-center mb-6">
                      <div className="text-center flex-1">
                        ...
                        {userData?.mood && MOODS.find(m => m.id === userData.mood) && <div className={`text-[10px] font-bold px-2 py-1 rounded-full mt-1 border ${MOODS.find(m => m.id === userData.mood).color}`}>{MOODS.find(m => m.id === userData.mood).icon} {MOODS.find(m => m.id === userData.mood).label}</div>}
                      </div>
                      <div className="text-slate-600 font-black px-4">VS</div>
                      <div className="text-center flex-1">
                        <div className="w-16 h-16 mx-auto rounded-full bg-slate-800 border-2 border-slate-700 overflow-hidden mb-2"><img src={partnerData?.avatarUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=partner&backgroundColor=1e293b`} alt="Partner" className="w-full h-full object-cover" /></div>
                        <div className="text-xs font-bold text-slate-300">{partnerData?.pseudo || 'Partenaire'}</div>
                        {partnerData?.mood && MOODS.find(m => m.id === partnerData.mood) ? <div className={`text-[10px] font-bold px-2 py-1 rounded-full mt-1 border ${MOODS.find(m => m.id === partnerData.mood).color}`}>{MOODS.find(m => m.id === partnerData.mood).icon} {MOODS.find(m => m.id === partnerData.mood).label}</div> : <div className="text-[10px] font-bold px-2 py-1 rounded-full mt-1 border border-slate-700 text-slate-500 bg-slate-800">Mystère...</div>}
                      </div>
                    </div>
                    <div className="pt-4 border-t border-slate-800">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {MOODS.map(mood => <button key={mood.id} onClick={() => updateMyMood(mood.id)} className={`p-2 rounded-xl text-xs font-bold border transition-all ${userData?.mood === mood.id ? mood.color : 'bg-slate-800 border-slate-700 text-slate-400'}`}>{mood.icon} {mood.label}</button>)}
                      </div>
                    </div>
                 </div>
                 ...
                 {sharedLikes.length === 0 ? <div className="text-center py-6 opacity-50 text-slate-400 text-sm border border-dashed border-slate-700 rounded-3xl p-6">Aucun match pour le moment.</div> : (
          ...
          <div className="animate-in fade-in duration-500 p-6 mt-4">
            ...
            <div className="flex flex-col items-center mb-10 relative">
               ...
               <div className="flex flex-col w-full max-w-xs items-center gap-3 mt-6">
                 ...
                 <button onClick={() => setShowIdeaModal(true)} className="flex items-center justify-center gap-2 bg-amber-600/20 text-amber-500 px-5 py-3 rounded-full text-xs font-black transition border border-amber-500/50 hover:bg-amber-600/40 shadow-lg w-full">
                   <Lightbulb size={14} /> Boîte à Idées (Suggestions)
                 </button>

                 <button onClick={() => setShowInstallTutorial(true)} className="flex items-center justify-center gap-2 bg-indigo-600/20 text-indigo-400 px-5 py-3 rounded-full text-xs font-black transition border border-indigo-500/50 hover:bg-indigo-600/40 shadow-lg w-full"><Smartphone size={14} /> Ajouter à l'écran d'accueil</button>
               </div>
            </div>
      ...
      {/* --- BOTTOM NAV --- */}
      <nav className="fixed bottom-0 w-full bg-slate-950/95 backdrop-blur-2xl border-t border-slate-900 px-2 flex justify-between items-center z-40" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 0.75rem)', paddingTop: '0.75rem' }}>
      ...
      {/* NOUVEAU : MODAL POPUP ADMIN */}
      {adminPopup && (
        <div className="fixed inset-0 z-[9999] bg-black/80 flex items-center justify-center p-6 backdrop-blur-sm animate-in fade-in">
          <div className="bg-slate-900 border border-rose-500 shadow-[0_0_30px_rgba(244,63,94,0.3)] rounded-3xl p-8 max-w-sm w-full text-center">
            <MessageSquare className="text-rose-500 mx-auto mb-4" size={40} />
            <h3 className="text-xl font-black text-white mb-4">Nouveau Message</h3>
            <p className="text-slate-300 mb-8 whitespace-pre-line text-lg">{adminPopup}</p>
            <button onClick={() => setAdminPopup(null)} className="w-full bg-rose-600 hover:bg-rose-500 text-white font-black py-4 rounded-xl transition">Compris !</button>
          </div>
        </div>
      )}

      {/* NOUVEAU : MODAL BOÎTE À IDÉES */}
      {showIdeaModal && (
        <div className="fixed inset-0 z-[200] bg-slate-950/95 backdrop-blur-xl flex flex-col animate-in slide-in-from-bottom-full duration-300">
          <header className="px-6 flex items-center justify-between border-b border-white/5 bg-slate-900/50" style={{ paddingTop: 'max(env(safe-area-inset-top), 1.25rem)', paddingBottom: '1.25rem' }}>
            <button onClick={() => setShowIdeaModal(false)} className="text-slate-400 p-2 bg-slate-800 rounded-full hover:text-white"><ArrowLeft size={20}/></button>
            <h2 className="font-black text-white flex items-center gap-2"><Lightbulb className="text-amber-500" size={18}/> Boîte à Idées</h2>
            <div className="w-9"/>
          </header>
          <div className="flex-1 overflow-y-auto p-6 flex flex-col items-center justify-start text-center pt-8">
            <Lightbulb size={64} className="text-amber-500 mx-auto mb-6 opacity-80" />
            <p className="text-slate-400 mb-8 max-w-sm">Vous avez une idée de fonctionnalité, de nouveau jeu, ou d'amélioration pour l'application ? Envoyez-la directement au Game Master !</p>
            <textarea 
               value={ideaText}
               onChange={(e) => setIdeaText(e.target.value)}
               placeholder="J'aimerais beaucoup que l'application puisse..."
               className="w-full max-w-sm bg-slate-900 border border-slate-800 focus:border-amber-500 p-4 rounded-2xl outline-none h-40 text-white text-base leading-relaxed resize-none transition"
            />
            <button onClick={handleSubmitIdea} disabled={!ideaText.trim()} className="w-full max-w-sm bg-amber-600 hover:bg-amber-500 disabled:opacity-50 disabled:bg-slate-800 text-white font-black py-4 rounded-xl mt-6 transition shadow-lg shadow-amber-900/20">
              Envoyer l'idée
            </button>
          </div>
        </div>
      ...
      {showPartnerProfile && partnerData && (
        <div className="fixed inset-0 z-[200] bg-slate-950 flex flex-col animate-in slide-in-from-bottom duration-300">
          <header className="px-6 flex items-center justify-between border-b border-white/5 bg-slate-900/50" style={{ paddingTop: 'max(env(safe-area-inset-top), 1.25rem)', paddingBottom: '1.25rem' }}>
            <button onClick={() => setShowPartnerProfile(false)} className="text-slate-400 p-2 bg-slate-800 rounded-full hover:text-white"><ArrowLeft size={20}/></button>
            <h2 className="font-black text-white tracking-tight">Profil de {partnerData.pseudo}</h2>
            <div className="w-9"/>
          </header>
          
          <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scroll">
            <div className="flex flex-col items-center">
               <div className="w-32 h-32 rounded-full border-4 border-indigo-500 bg-slate-900 mb-4 overflow-hidden shadow-[0_0_30px_rgba(99,102,241,0.3)]">
                  <img src={partnerData.avatarUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=partner&backgroundColor=1e293b`} alt="Avatar" className="w-full h-full object-cover" />
               </div>
               <h2 className="text-3xl font-black text-white mb-2">{partnerData.pseudo}</h2>
               <p className="text-indigo-200 text-sm text-center max-w-xs">{partnerData.bio}</p>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 text-center shadow-lg">
                <Heart className="mx-auto text-rose-500 mb-2" size={28} />
                <div className="text-3xl font-black text-white">{partnerData.likes?.length || 0}</div>
                <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1">Favoris</div>
              </div>
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 text-center shadow-lg">
                <FolderPlus className="mx-auto text-indigo-500 mb-2" size={28} />
                <div className="text-3xl font-black text-white">{partnerCustomPositions.length}</div>
                <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1">Créations</div>
              </div>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 flex items-center gap-4">
               ...
               <div>
                 <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Dernière activité</div>
                 <div className="text-white font-bold">
                   {partnerData.lastActive && (Date.now() - partnerData.lastActive < 5 * 60 * 1000) 
                     ? <span className="text-emerald-400 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span> En ligne actuellement</span>
                     : "Hors ligne"}
                 </div>
               </div>
            </div>
      ...
      {isChatOpen && (
        <div className="fixed inset-0 z-[200] bg-slate-950 flex flex-col animate-in slide-in-from-right duration-300">
          <header className="px-6 flex items-center justify-between border-b border-white/5 bg-slate-950/90 backdrop-blur-xl z-10 shrink-0" style={{ paddingTop: 'max(env(safe-area-inset-top), 1.25rem)', paddingBottom: '1.25rem' }}>
            ...
            <div className="flex flex-col items-center"><h2 className="font-black text-white tracking-tight flex items-center gap-2">{partnerData?.pseudo || 'Partenaire'}</h2><span className="text-[10px] text-emerald-400 font-bold uppercase tracking-widest">Connexion sécurisée</span></div>
          ...
          <div className="flex-1 overflow-y-auto p-4 custom-scroll space-y-4 flex flex-col bg-slate-950" style={{ WebkitOverflowScrolling: 'touch' }}>
            {messages.length === 0 ? <div className="flex-1 flex flex-col items-center justify-center text-slate-500 text-sm space-y-4 opacity-50"><MessageSquare size={48} className="text-slate-700" /><p>Envoyez votre premier message...</p></div> : (
               messages.map((msg, i) => {
                 const isMe = msg.uid === user?.uid;
                 return (
                   <div key={msg.id || i} className={`max-w-[80%] flex ${isMe ? 'ml-auto justify-end' : 'mr-auto justify-start'}`}>
                     <div className={`px-4 py-3 text-sm ${isMe ? 'bg-rose-600 text-white rounded-l-2xl rounded-tr-2xl' : 'bg-slate-800 text-slate-200 rounded-r-2xl rounded-tl-2xl'}`}>{msg.text}</div>
                   </div>
                 );
               })
            )}
          ...
          <form onSubmit={handleSendMessage} className="p-4 bg-slate-950/80 backdrop-blur-xl border-t border-slate-900 shrink-0 flex gap-2 items-end" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 1rem)' }}>
             <textarea value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Écrire un message secret..." className="flex-1 bg-slate-900 border border-slate-800 text-white p-3 rounded-2xl outline-none text-base resize-none max-h-32 min-h-[50px] custom-scroll" rows="1" onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage(e); } }} />
             <button type="submit" disabled={!newMessage.trim()} className="bg-rose-600 p-3.5 rounded-2xl text-white disabled:opacity-50 disabled:bg-slate-800 transition-colors shrink-0"><Send size={20} className={newMessage.trim() ? 'translate-x-0.5' : ''} /></button>
          </form>
        </div>
      )}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scroll" style={{ WebkitOverflowScrolling: 'touch', paddingBottom: 'calc(env(safe-area-inset-bottom) + 100px)' }}>
            <div className="flex flex-col items-center space-y-4">
              <div className="relative">
                <div className="w-28 h-28 rounded-full border-4 border-slate-800 bg-slate-900 overflow-hidden shadow-xl"><img src={profileForm.avatarUrl} alt="Avatar" className="w-full h-full object-cover" /></div>
                <button onClick={generateNewAvatar} className="absolute bottom-0 right-0 bg-rose-600 text-white p-2 rounded-full shadow-lg border-2 border-slate-950 hover:scale-110 transition"><RefreshCw size={16} /></button>
              </div>
              <span className="text-xs text-slate-400 font-medium">Avatar aléatoire</span>
            </div>
            <div className="space-y-4">
              <div><label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Pseudo</label><input className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500 p-4 rounded-xl outline-none text-white text-base" placeholder="Votre pseudonyme" value={profileForm.pseudo} onChange={(e) => setProfileForm({...profileForm, pseudo: e.target.value})} maxLength={20} /></div>
              <div><label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Biographie</label><textarea className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500 p-4 rounded-xl outline-none h-32 text-base leading-relaxed text-slate-300 resize-none" placeholder="Dites-en plus sur vous..." value={profileForm.bio} onChange={(e) => setProfileForm({...profileForm, bio: e.target.value})} maxLength={150} /></div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 custom-scroll" style={{ WebkitOverflowScrolling: 'touch', paddingBottom: '20px' }}>
            {showDeleteConfirm && (
              <div className="bg-rose-900/20 border border-rose-500/30 rounded-3xl p-6 mb-6 text-center animate-in zoom-in">
                ...
                <div className="flex gap-2"><button onClick={handleDeletePosition} className="flex-1 bg-rose-600 text-white py-3 rounded-xl font-bold">Oui, Supprimer</button><button onClick={() => setShowDeleteConfirm(false)} className="flex-1 bg-slate-800 text-white py-3 rounded-xl font-bold">Annuler</button></div>
              </div>
            )}
            <div className={`space-y-6 ${discreetMode ? 'blur-md opacity-50 select-none' : ''}`}>
               <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 relative overflow-hidden">
                  ...
                  <h4 className="text-rose-400 font-bold text-xs uppercase tracking-widest mb-3 flex items-center gap-2"><Sparkles size={14}/> La Posture</h4>
               ...
               {selectedPosition.v && (
                 <div className="bg-indigo-900/20 border border-indigo-500/20 rounded-3xl p-6"><h4 className="text-indigo-400 font-bold text-xs uppercase tracking-widest mb-3 flex items-center gap-2"><RefreshCw size={14}/> Variante & Astuce</h4><p className="text-slate-300 text-sm leading-relaxed">{applyDiscreet(selectedPosition.v)}</p></div>
               )}
            </div>
            <div className="grid grid-cols-2 gap-4 my-8">
              <div className="bg-slate-900/40 p-5 rounded-3xl border border-slate-800 text-center"><span className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Physique</span><div className="flex justify-center gap-1.5">{[...Array(5)].map((_, i) => <div key={i} className={`w-3 h-3 rounded-full ${i < selectedPosition.diff ? 'bg-indigo-500' : 'bg-slate-800'}`}/>)}</div></div>
              <div className="bg-slate-900/40 p-5 rounded-3xl border border-slate-800 text-center"><span className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Intensité</span><div className="flex justify-center gap-1.5">{[...Array(5)].map((_, i) => <div key={i} className={`w-3 h-3 rounded-full ${i < selectedPosition.spice ? 'bg-rose-500' : 'bg-slate-800'}`}/>)}</div></div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[200] bg-slate-950/95 backdrop-blur-xl flex flex-col animate-in slide-in-from-bottom-full duration-300">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scroll" style={{ WebkitOverflowScrolling: 'touch', paddingBottom: '20px' }}>
            <div className="flex items-center justify-between bg-slate-900 border border-slate-800 p-4 rounded-2xl">
               <div><span className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-1">Visibilité</span><span className={`text-sm font-bold ${newPos.shared ? 'text-emerald-400' : 'text-slate-400'}`}>{newPos.shared ? 'Partagée avec mon partenaire' : 'Privée (Moi uniquement)'}</span></div>
               <button onClick={() => setNewPos({...newPos, shared: !newPos.shared})} className={`w-14 h-8 rounded-full relative transition-colors ${newPos.shared ? 'bg-emerald-500' : 'bg-slate-700'}`}><div className={`w-6 h-6 rounded-full bg-white absolute top-1 transition-transform ${newPos.shared ? 'translate-x-7' : 'translate-x-1'}`}/></button>
            </div>
            <div className="space-y-1"><label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Nom de la position</label><input className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500 p-4 rounded-2xl outline-none text-white text-base" placeholder="Ex: Le Volcan..." value={newPos.name} onChange={(e) => setNewPos({...newPos, name: e.target.value})} /></div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Catégorie</label>
              <select className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500 p-4 rounded-2xl outline-none text-white text-base appearance-none" value={newPos.cat} onChange={(e) => setNewPos({...newPos, cat: e.target.value})}>{displayCategories.map(c => <option key={c.id} value={c.id}>{c.id}</option>)}<option value="NEW">+ Créer une nouvelle catégorie...</option></select>
              {newPos.cat === 'NEW' && <div className="mt-3 animate-in fade-in slide-in-from-top-2"><input className="w-full bg-indigo-900/20 border border-indigo-500/50 focus:border-indigo-400 p-4 rounded-2xl outline-none text-indigo-300 text-base placeholder:text-indigo-900/50" placeholder="Nom de votre nouvelle catégorie" value={newPos.newCat} onChange={(e) => setNewPos({...newPos, newCat: e.target.value})} autoFocus /></div>}
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl"><label className="text-[9px] font-black text-slate-500 uppercase block mb-3">Physique ({newPos.diff}/5)</label><input type="range" min="1" max="5" value={newPos.diff} onChange={(e) => setNewPos({...newPos, diff: parseInt(e.target.value)})} className="w-full accent-indigo-500" /></div>
              <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl"><label className="text-[9px] font-black text-slate-500 uppercase block mb-3">Intensité ({newPos.spice}/5)</label><input type="range" min="1" max="5" value={newPos.spice} onChange={(e) => setNewPos({...newPos, spice: parseInt(e.target.value)})} className="w-full accent-rose-500" /></div>
            </div>
            <div className="space-y-1"><label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Description de la Posture</label><textarea className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500 p-5 rounded-2xl outline-none h-32 text-base leading-relaxed text-slate-300 resize-none" placeholder="Décrivez comment se placer..." value={newPos.desc} onChange={(e) => setNewPos({...newPos, desc: e.target.value})} /></div>
            <div className="space-y-1"><label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Variante (Optionnel)</label><textarea className="w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 p-5 rounded-2xl outline-none h-24 text-base leading-relaxed text-slate-300 resize-none" placeholder="Une astuce ou variante pour pimenter..." value={newPos.v} onChange={(e) => setNewPos({...newPos, v: e.target.value})} /></div>
          </div>
          <div className="p-6 bg-slate-950 border-t border-slate-900" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 1.5rem)' }}>
             <button onClick={handleSavePosition} className="w-full bg-rose-600 hover:bg-rose-500 text-white py-4 rounded-xl font-black uppercase tracking-widest shadow-lg shadow-rose-900/20 transition-all active:scale-[0.98]">{editPosId ? 'Enregistrer les modifications' : (newPos.shared ? 'Créer et Partager' : 'Créer Secrètement')}</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[200] bg-slate-950 flex flex-col animate-in slide-in-from-bottom duration-300">
          ...
          <div className="flex-1 overflow-y-auto p-6 custom-scroll space-y-6" style={{ WebkitOverflowScrolling: 'touch', paddingBottom: 'calc(env(safe-area-inset-bottom) + 20px)' }}>
            <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-[2rem] space-y-4">
              ...
              <ol className="list-decimal pl-5 space-y-2 text-sm text-slate-300 font-medium"><li>Ouvrez l'application <b>Safari</b> et allez sur <code>kama-sync.vercel.app</code></li><li>Appuyez sur l'icône <b>Partager</b> en bas de l'écran.</li><li>Faites défiler et choisissez <b>Sur l'écran d'accueil</b>.</li><li>Appuyez sur <b>Ajouter</b>.</li></ol>
            </div>
            <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-[2rem] space-y-4">
              <h3 className="text-base font-black text-white flex items-center gap-2">🤖 Pour Android</h3>
              <ol className="list-decimal pl-5 space-y-2 text-sm text-slate-300 font-medium"><li>Ouvrez l'application <b>Chrome</b> et allez sur <code>kama-sync.vercel.app</code></li><li>Appuyez sur les <b>trois petits points</b> en haut à droite.</li><li>Sélectionnez <b>Ajouter à l'écran d'accueil</b>.</li><li>Appuyez sur <b>Installer</b>.</li></ol>
            </div>
          </div>
        </div>
      )}
