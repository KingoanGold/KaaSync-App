...
export default function App() {
  ...
  // Vérifier si les notifications système sont déjà activées au chargement
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsubUser = onSnapshot(userRef, (snap) => {
      if (!snap.exists()) {
        const initial = { 
          uid: user.uid, pseudo: 'Anonyme', bio: 'Explorateur de sensations...',
          avatarUrl: `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.uid}&backgroundColor=1e293b`,
          likes: [], pairCode: Math.random().toString(36).substring(2, 8).toUpperCase(), partnerUid: null,
          mood: 'playful', lastIntimacy: 0, pingToPartner: 0
        };
        ...
        if (data.partnerUid) {
          onSnapshot(doc(db, 'artifacts', appId, 'users', data.partnerUid), (pSnap) => {
            if (pSnap.exists()) {
               ...
               // LOGIQUE DU SIGNAL DISCRET (PING) + NOTIF PUSH
               if (pData.pingToPartner && pData.pingToPartner > lastSeenPing) {
                  if (Date.now() - pData.pingToPartner < 60000) {
                     ...
                     // NOUVEAU: DECLENCHER LA NOTIFICATION SYSTEME SI AUTORISEE
                     if ("Notification" in window && Notification.permission === "granted") {
                        try {
                           new Notification("KamaSync 🔥", {
                              body: `${pData.pseudo || 'Votre partenaire'} a très envie de vous...`,
                           });
                        } catch(e) { /* Fallback silence pour certains navigateurs iOS sans ServiceWorker */ }
                     }
                  }
  ...
  const filteredPositions = useMemo(() => {
    let result = allPositions.filter(pos => {
      return (pos.name.toLowerCase().includes(searchQuery.toLowerCase())) &&
             (filterSpice === 0 || pos.spice === filterSpice) &&
             (filterPhysique === 0 || pos.diff === filterPhysique) &&
             (filterCat === 'Toutes' || pos.cat === filterCat);
    });
  ...
  // NOUVEAU: DEMANDE DE PERMISSION NOTIFICATIONS
  const requestPushNotifications = async () => {
    if (!("Notification" in window)) {
      notify("Ton navigateur ne supporte pas les notifications système.", "❌");
      return;
    }
    try {
      ...
      if (permission === "granted") {
        setPushEnabled(true);
        notify("Notifications push activées !", "🔔");
      } else {
        notify("Tu as refusé les notifications.", "⚠️");
      }
    } catch(e) {
  ...
  // --- NOUVELLE LOGIQUE GOOGLE LOGIN AVEC MIGRATION DE DONNEES ---
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // SI LE COMPTE GOOGLE EXISTE DEJA, ON FUSIONNE LES DONNEES
      if (error.code === 'auth/credential-already-in-use') {
         const currentPartnerUid = userData?.partnerUid;
         const currentLikes = userData?.likes || [];
         const myCustoms = [...myCustomPositions];

         // Connexion au compte existant (l'UID va changer)
         const result = await signInWithPopup(auth, provider);
         const newUid = result.user.uid;
         const newRef = doc(db, 'artifacts', appId, 'users', newUid);
         
         try {
           // On force l'ajout du partenaire sur le nouveau profil
           if (currentPartnerUid) {
             await setDoc(newRef, { partnerUid: currentPartnerUid }, { merge: true });
           }
           // On transfère les likes
           if (currentLikes.length > 0) {
             await setDoc(newRef, { likes: arrayUnion(...currentLikes) }, { merge: true });
           }
           // On transfère les créations personnalisées
           if (myCustoms.length > 0) {
             const newCustomCol = collection(db, 'artifacts', appId, 'users', newUid, 'customPositions');
             myCustoms.forEach(async (pos) => {
                const { id, isMine, isCustom, isPartner, ...cleanPos } = pos;
                await setDoc(doc(newCustomCol, id), cleanPos);
             });
           }
           notify("Compte récupéré et données fusionnées ! 🎉", "✅");
         } catch(e) {
           notify("Connecté, mais la fusion a partiellement échoué.", "⚠️");
         }
      } else {
  ...
  const handleLinkPartner = async () => {
    ...
    } catch (error) { notify("Erreur lors de la recherche.", "❌"); }
  };

  const handleUnlinkPartner = async () => {
    if (!user) return;
    if (window.confirm("⚠️ Voulez-vous vraiment vous séparer de ce partenaire ?")) {
          ...
          <div className="animate-in fade-in duration-500 p-6 mt-4">
             ...
             <div className="grid grid-cols-1 gap-4">
               <button onClick={() => setActiveGame('truthOrDare')} className="bg-slate-900 border border-slate-800 rounded-3xl p-5 text-left flex items-center justify-between group hover:bg-slate-800 transition">
                 <div><h3 className="font-bold text-white flex items-center gap-2 mb-1"><Zap size={18} className="text-rose-500"/> Action ou Vérité</h3><p className="text-xs text-slate-400">Des confessions intimes et des défis charnels.</p></div><ChevronRight className="text-slate-700 group-hover:text-white" />
               </button>
               <button onClick={() => setActiveGame('loveDice')} className="bg-slate-900 border border-slate-800 rounded-3xl p-5 text-left flex items-center justify-between group hover:bg-slate-800 transition">
                 <div><h3 className="font-bold text-white flex items-center gap-2 mb-1"><Dices size={18} className="text-amber-500"/> Dés de l'Amour</h3><p className="text-xs text-slate-400">Laissez le hasard dicter vos caresses.</p></div><ChevronRight className="text-slate-700 group-hover:text-white" />
               </button>
               <button onClick={() => setActiveGame('scenario')} className="bg-slate-900 border border-slate-800 rounded-3xl p-5 text-left flex items-center justify-between group hover:bg-slate-800 transition">
                 <div><h3 className="font-bold text-white flex items-center gap-2 mb-1"><Shuffle size={18} className="text-purple-500"/> Scénario Aléatoire</h3><p className="text-xs text-slate-400">Lieu + Rôle + Twist inattendu.</p></div><ChevronRight className="text-slate-700 group-hover:text-white" />
               </button>
               <button onClick={() => setActiveGame('roulette')} className="bg-slate-900 border border-slate-800 rounded-3xl p-5 text-left flex items-center justify-between group hover:bg-slate-800 transition">
                 <div><h3 className="font-bold text-white flex items-center gap-2 mb-1"><Timer size={18} className="text-pink-500"/> Roulette Préliminaires</h3><p className="text-xs text-slate-400">Des tâches sensorielles pour faire monter le désir.</p></div><ChevronRight className="text-slate-700 group-hover:text-white" />
               </button>
               <button onClick={() => setActiveGame('secretChallenge')} className="bg-slate-900 border border-slate-800 rounded-3xl p-5 text-left flex items-center justify-between group hover:bg-slate-800 transition">
                 <div><h3 className="font-bold text-white flex items-center gap-2 mb-1"><Gift size={18} className="text-emerald-500"/> Défi Secret (24h)</h3><p className="text-xs text-slate-400">Un défi à accomplir en cachette.</p></div><ChevronRight className="text-slate-700 group-hover:text-white" />
               </button>
             </div>
          </div>
        )}
                   ...
                   <div className="grid grid-cols-1 gap-3">
                     {sharedLikes.map(pos => (
                       <div key={pos.id} onClick={() => setSelectedPosition(pos)} className="bg-slate-900 p-4 rounded-xl flex items-center justify-between cursor-pointer border border-amber-500/20">
                         <div className="flex items-center gap-3"><Flame className="text-amber-500" size={18} /><span className="font-bold text-sm text-white">{discreetMode ? "Masqué" : pos.name}</span></div><ChevronRight className="text-slate-600" size={16} />
                       </div>
                     ))}
                   </div>
                 )}
          ...
          <div className="animate-in fade-in duration-500 p-6 mt-4">
            ...
            <div className="flex flex-col items-center mb-10 relative">
               <div className="w-24 h-24 rounded-full border-4 border-slate-800 bg-slate-900 mb-4 overflow-hidden shadow-xl relative group cursor-pointer"
                    onClick={() => { setProfileForm({ pseudo: userData?.pseudo || '', bio: userData?.bio || '', avatarUrl: userData?.avatarUrl || '' }); setIsEditingProfile(true); }}>
               ...
               <div className="flex flex-col w-full max-w-xs items-center gap-3 mt-6">
                 <button onClick={() => { setProfileForm({ pseudo: userData?.pseudo || '', bio: userData?.bio || '', avatarUrl: userData?.avatarUrl || '' }); setIsEditingProfile(true); }} className="flex items-center justify-center gap-2 bg-slate-800 text-white px-5 py-3 rounded-full text-xs font-bold transition border border-slate-700 hover:bg-slate-700 w-full"><Edit2 size={14} /> Modifier mon profil</button>
                 
                 {/* BOUTON GOOGLE AMÉLIORÉ */}
                 {user?.isAnonymous && (
                   <button onClick={handleGoogleLogin} className="flex items-center justify-center gap-2 bg-white text-slate-900 px-5 py-3 rounded-full text-xs font-black transition hover:bg-slate-200 shadow-lg shadow-white/10 w-full"><LogIn size={14} /> Sauvegarder (Google)</button>
                 )}
                 ...
                 {!pushEnabled && (
                   <button onClick={requestPushNotifications} className="flex items-center justify-center gap-2 bg-amber-500/20 text-amber-500 px-5 py-3 rounded-full text-xs font-black transition border border-amber-500/50 hover:bg-amber-500/40 shadow-lg w-full">
                     <BellRing size={14} /> Activer les notifications push
                   </button>
                 )}
               </div>
            </div>
        ...
        <div className="fixed inset-0 z-[200] bg-slate-950 flex flex-col animate-in slide-in-from-right duration-300">
          <header className="px-6 flex items-center justify-between border-b border-white/5 bg-slate-950/90 backdrop-blur-xl z-10 shrink-0" style={{ paddingTop: 'max(env(safe-area-inset-top), 1.25rem)', paddingBottom: '1.25rem' }}>
            ...
            <div className="flex flex-col items-center">
              <h2 className="font-black text-white tracking-tight flex items-center gap-2">{partnerData?.pseudo || 'Partenaire'}</h2>
          ...
          <div className="flex-1 overflow-y-auto p-4 custom-scroll space-y-4 flex flex-col bg-slate-950" style={{ WebkitOverflowScrolling: 'touch' }}>
            {messages.length === 0 ? (
               <div className="flex-1 flex flex-col items-center justify-center text-slate-500 text-sm space-y-4 opacity-50"><MessageSquare size={48} className="text-slate-700" /><p>Envoyez votre premier message...</p></div>
            ) : (
               messages.map((msg, i) => {
