...
const POSITIONS_DATA = [
  ...
  { n: "Le V incliné", c: "De côté", d: 3, s: 3, desc: "Les deux partenaires sont sur le flanc, mais leurs bustes s'éloignent pour former un V, seuls leurs bassins restent connectés au centre.", v: "Variante : Le partenaire avant regarde par-dessus son épaule pour maintenir le contact visuel." },
...
export default function App() {
  ...
  // Séparation des créations (Miennes / Partenaire)
  ...
  // State pour la création/édition
  const [editPosId, setEditPosId] = useState(null);
  // NOUVEAU : Champ 'shared' par défaut à true pour partager avec le partenaire
    ...
    const unsubUser = onSnapshot(userRef, (snap) => {
      if (!snap.exists()) {
        ...
        if (data.partnerUid) {
          onSnapshot(doc(db, 'artifacts', appId, 'users', data.partnerUid), (pSnap) => {
            if (pSnap.exists()) {
               ...
               if (pData.pingToPartner && pData.pingToPartner > lastSeenPing) {
                  // Si le ping a moins d'une minute, on l'affiche
          ...
          // NOUVEAU : Récupération des créations du partenaire (Uniquement celles qui sont PARTAGÉES)
          ...
          unsubPartnerCustom = onSnapshot(partnerCustomCol, (cSnap) => {
            // On filtre pour ne garder que celles où shared n'est pas "false"
  ...
  const handleOpenEdit = (pos) => {
    ...
    setNewPos({
      ...
      diff: pos.diff,
      spice: pos.spice,
      shared: pos.shared !== false // Défaut true sauf si explicitement false
    });
    setEditPosId(pos.id);
  ...
  return (
    <div className="h-screen bg-slate-950 text-slate-100 flex flex-col overflow-hidden font-sans">
      ...
      {/* MAIN CONTENT AREA */}
      <main className="flex-1 overflow-y-auto custom-scroll pb-32 relative">
            ...
            <div className="px-6 mb-8 space-y-4">
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full bg-slate-900 border border-slate-800 rounded-3xl py-4 pl-14 pr-4 outline-none focus:border-rose-500 text-sm"
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
                  <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-slate-900 border border-slate-800 rounded-xl px-4 py-2 text-[10px] font-bold text-emerald-400 min-w-[120px]">
                    <option value="az">Trier : A-Z</option>
                    <option value="spice">Trier : Plus épicé</option>
                    <option value="diff">Trier : Moins physique</option>
                  </select>
                  <select value={filterCat} onChange={(e) => setFilterCat(e.target.value)} className="bg-slate-900 border border-slate-800 rounded-xl px-4 py-2 text-[10px] font-bold text-slate-300 min-w-[120px]">
                    ...
                    {categoryPositions.map(pos => {
                      ...
                      return (
                        <div key={pos.id} onClick={() => setSelectedPosition(pos)} className={`relative snap-center shrink-0 w-48 bg-gradient-to-br ${category.color} border ${isMatch ? 'border-amber-500/50' : 'border-white/5'} rounded-[2rem] p-5 cursor-pointer hover:scale-[1.02] transition-all`}>
                          ...
                          {/* Badges d'appartenance pour les créations */}
             ...
             <div className="grid grid-cols-1 gap-4">
               ...
               <button onClick={() => setActiveGame('secretChallenge')} className="bg-slate-900 border border-slate-800 rounded-3xl p-5 text-left flex items-center justify-between group hover:bg-slate-800 transition">
                 <div>
                  ...
                  <p className="text-xs text-slate-400">Un défi à réaliser sans prévenir votre partenaire.</p>
                 </div>
                 <ChevronRight className="text-slate-700 group-hover:text-white" />
               </button>
             </div>
        ...
        {activeTab === 'jeux' && activeGame && (
          <div className="absolute inset-0 bg-slate-950 z-10 animate-in slide-in-from-right duration-300 flex flex-col">
            <header className="px-6 py-5 flex items-center justify-between border-b border-white/5 bg-slate-900/50">
             ...
             {!userData?.partnerUid ? (
               <div className="bg-slate-900 border border-slate-800 rounded-[2rem] p-8 text-center mb-8">
                 ...
                 <div className="flex gap-2">
                   <input 
                     className="flex-1 bg-slate-800 border-none rounded-xl text-center font-mono uppercase text-white outline-none px-4"
                     placeholder="CODE PARTENAIRE"
                     value={partnerCodeInput}
                     onChange={(e) => setPartnerCodeInput(e.target.value.toUpperCase())}
                     maxLength={6}
                   />
                   <button onClick={handleLinkPartner} className="bg-emerald-600 px-6 py-3 rounded-xl font-bold text-white text-xs">LIER</button>
                 </div>
               </div>
             ) : (
      ...
      {isEditingProfile && (
        <div className="fixed inset-0 z-[200] bg-slate-950/95 backdrop-blur-xl flex flex-col animate-in slide-in-from-bottom-full duration-300">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8 pb-32 custom-scroll">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input 
                  className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500 p-4 rounded-xl outline-none text-white text-sm" 
              ...
              <div>
                ...
                <textarea 
                  className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500 p-4 rounded-xl outline-none h-32 text-sm leading-relaxed text-slate-300 resize-none" 
      ...
      {selectedPosition && (
        <div className="fixed inset-0 z-[200] bg-slate-950 flex flex-col animate-in slide-in-from-bottom duration-300">
          <div className="relative pt-12 pb-8 px-6 bg-gradient-to-b from-rose-950/40 to-slate-950 shrink-0">
            <button onClick={() => {setSelectedPosition(null); setShowDeleteConfirm(false);}} className="absolute top-6 left-6 text-slate-400 bg-slate-900/50 p-2 rounded-full backdrop-blur-md hover:bg-slate-800 transition"><ArrowLeft size={20}/></button>
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-32 custom-scroll">
      ...
      {isCreating && (
        <div className="fixed inset-0 z-[200] bg-slate-950/95 backdrop-blur-xl flex flex-col animate-in slide-in-from-bottom-full duration-300">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 pb-32 custom-scroll">
            ...
            <div className="space-y-1">
              ...
              <input className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500 p-4 rounded-2xl outline-none text-white text-sm" placeholder="Ex: Le Volcan..." value={newPos.name} onChange={(e) => setNewPos({...newPos, name: e.target.value})} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <select className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500 p-4 rounded-2xl outline-none text-white text-sm appearance-none" value={newPos.cat} onChange={(e) => setNewPos({...newPos, cat: e.target.value})}>
                {displayCategories.map(c => <option key={c.id} value={c.id}>{c.id}</option>)}
              ...
              {newPos.cat === 'NEW' && (
                <div className="mt-3 animate-in fade-in slide-in-from-top-2">
                  <input className="w-full bg-indigo-900/20 border border-indigo-500/50 focus:border-indigo-400 p-4 rounded-2xl outline-none text-indigo-300 text-sm placeholder:text-indigo-900/50" placeholder="Nom de votre nouvelle catégorie" value={newPos.newCat} onChange={(e) => setNewPos({...newPos, newCat: e.target.value})} autoFocus />
                </div>
              )}
            </div>
            ...
            <div className="space-y-1">
              ...
              <textarea className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500 p-5 rounded-2xl outline-none h-32 text-sm leading-relaxed text-slate-300 resize-none" placeholder="Décrivez comment se placer..." value={newPos.desc} onChange={(e) => setNewPos({...newPos, desc: e.target.value})} />
            </div>

            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Variante (Optionnel)</label>
              <textarea className="w-full bg-slate-900 border border-slate-800 focus:border-indigo-500 p-5 rounded-2xl outline-none h-24 text-sm leading-relaxed text-slate-300 resize-none" placeholder="Une astuce ou variante pour pimenter..." value={newPos.v} onChange={(e) => setNewPos({...newPos, v: e.target.value})} />
            </div>
      ...
      {selectedTip && (
        <div className="fixed inset-0 z-[200] bg-slate-950 flex flex-col animate-in slide-in-from-bottom duration-300">
          <header className="px-6 py-5 bg-slate-900/50"><button onClick={() => setSelectedTip(null)} className="text-slate-400 bg-slate-800 p-2 rounded-full hover:bg-slate-700 transition"><ArrowLeft size={20}/></button></header>
          <div className="flex-1 overflow-y-auto px-6 py-8 custom-scroll pb-32">
